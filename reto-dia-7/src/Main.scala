...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    val usuarios = Set(estudiante1, estudiante2, estudiante3, estudiante4,
      invitado1, invitado2, administrador1, administrador2)
    ...
    usuarios.foreach {
      ...
      case Estudiante(nombre, password, _, _, _) =>
        ...
        estaVacio(nombre) match {
          case true => println(errorCredenciales)
          case false => estaVacio(password) match {
            case true => println(errorCredenciales)
            case false => println(s"Bienvenid@ a OpenVitae, $nombre")
          }
        }
